...
def rank_front(nodes: list, N: int, player_id) -> list:
    rank_frontlist=[0]*(N+1)
    for j in range(N) :
        if nodes[j].belong == -1 :
            rank_frontlist[j] += 1
        elif nodes[j].belong == player_id :
            t = nodes[j].get_next()
            for item in t :
                if nodes[item].belong != player_id :
                    rank_frontlist[item]=1
    return rank_frontlist
...
def assessMarch(nodes: list, node: int, edge: tuple, scores: list, player_id: int, rank_frontlist: list) -> float:
    '''
    防止暴毙，先写每一步的贪心
    '''
    a=0.8
    ...
    manpowerscore = 0
    manpower = nodes[node].power[player_id]
    aimnode, marchpower = nodes[edge[0]], edge[1] #aimnode为节点
...
def decideMarch(nodes: list, node: int, scores: list, actions: list, player_id: int, rank_frontlist: list) -> None:
    '''
    ...
    for x in next_nodes:
        ...
        for y in range(b+1):
            assessPoint = assessMarch(
                nodes, node, (x, power_bin*y), scores, player_id, rank_frontlist)
...
def decideMarches(nodes: list, scores: list, player_id: int, N: int, rank_frontlist: list) -> list:
    '''
    ...
    counted = {root}
    actions = []
    decideMarch(nodes, root, scores, actions, player_id, rank_frontlist)
    while counting:
        # print(counting)
        newcounting = set()
        for node in counting:
            for newnode in nodes[node].get_next():
                ...
                if nodes[newnode].belong == player_id:
                    decideMarch(nodes, newnode, scores, actions, player_id, rank_frontlist)
...
class player_class:
    ...
    def player_func(self, map_info: GameMap):
        ...
        N = map_info.N
        rank_frontlist=rank_front(nodes,N,player_id)
        # print('1')
        scores = countScores(nodes, N , player_id)
        #print("================================================")
        #print(scores)
        #print("========================================")
        actions = decideMarches(nodes, scores, self.player_id, N,rank_frontlist)
        #print(actions)
        return actions
